...
async def _vectorize(
    ...
    try:
        ...
        for offset in range(0, len(rows), config.batch_size):
            ...
            persisted = (
                await store.load_vectors([row["id"] for row in batch]) if exists else {}
            )
            pending = []
            for row in batch:
                keys = {
                ...
                if str(row["id"]) in persisted:
                    ...
                    item = await upsert_record(
                        RunItem,
                        keys,
                        {"dataset_id": dataset_id, "status": "running", "error": None},
                    )
            ...
            try:
                ...
                for index, (row, _) in enumerate(pending):
                    ...
                    if row.get("representation_id"):
                        metadata["representation_id"] = str(row["representation_id"])
                    nodes.append(
                        TextNode(
                            id_=str(row["id"]),
                            text=row["text"],
                            metadata=metadata,
                            embedding=dense_vectors[index] if dense_vectors else None,
